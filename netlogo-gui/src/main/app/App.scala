...
object App {
  ...
  def mainWithAppHandler(args: Array[String], appHandler: Object) {
    ...
    val bootAs3D = Version.is3DInternal
    Splash.beginSplash(Version.getCurrent(bootAs3D)) // also initializes AWT
  ...
  case class CommandLineParameters(openTarget: OpenTarget, loggingConfig: Option[String], loggingDirectory: Option[String])
  ...
  private def processCommandLineArguments(args: Array[String]): CommandLineParameters = {
    ...
    // set) - ST 4/1/02
    var i = 0
    var params = CommandLineParameters(EmptyModel, None, None)
    ...
    while(moreTokens) {
      ...
      if (token == "--events") org.nlogo.window.Event.logEvents = true;
        ...
        params = params.copy(openTarget = CommandLineModel(modelFile.getAbsolutePath(), false))
      }
      else if (token == "--magic") {
        ...
        params = params.copy(openTarget = CommandLineModel(modelFile.getAbsolutePath(), true))
      }
    }
    params
  }
